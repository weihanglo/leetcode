...
// If you want a challenge, please do not see below and ask yourself what are
// the possible input cases.
//
// Notes: It is intended for this problem to be specified vaguely (ie, 
// no given input specs). You are responsible to gather all the input requirements up front. /
//
...
var myAtoi = function(str) {
  const d = /^([+-])?(\d+)/.exec(str.trim())
  if (d && d[2]) {
    const int = parseInt(d[2])
    const bit32 = Math.pow(2, 31)
    return d[1] === '-'
      ? -int >= -bit32 ? -int : -bit32
      : int < bit32 ? int : bit32 - 1
  }
  return 0
};
